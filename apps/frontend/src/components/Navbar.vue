...
const { isAuthenticated, login, logout, currentUser, toggleAuthModal } = useAuthStore()
    ...
    <nav class="bg-neutral-primary">
      <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl p-4">
        ...
        <div class="flex items-center space-x-6 rtl:space-x-reverse">
          <a href="tel:5541251234" class="text-sm text-body hover:underline">{{ user }}</a>
          <a v-if="!isAuthenticated" @click="toggleAuthModal" href="#" class="text-sm font-medium text-fg-brand hover:underline">Login</a>
          <a v-else href="#" class="text-sm font-medium text-fg-brand hover:underline">Logout</a>
        </div>
      </div>
    </nav>
