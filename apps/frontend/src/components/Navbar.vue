<script setup lang="ts">
import { computed } from 'vue'
import { useAuthStore } from '@/stores/auth'

const authStore = useAuthStore()

// Computed property for user display
const userDisplay = computed(() => {
  if (!authStore.currentUser) return 'Guest'
  return authStore.currentUser.email || authStore.currentUser.id || 'User'
})

// Handle logout
const handleLogout = async () => {
  await authStore.logout()
}
</script>

<template>
  <header class="fixed w-full z-20 top-0 start-0">
    <nav class="bg-neutral-primary">
      <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl p-4">
        <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
          <div class="w-8 h-8 bg-orange-400 rounded-lg flex items-center justify-center">
            <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
              <path
                d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z"
              />
              <path
                fill-rule="evenodd"
                d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z"
                clip-rule="evenodd"
              />
            </svg>
          </div>
          <span class="self-center text-xl text-heading font-semibold whitespace-nowrap"
            >Trading</span
          >
        </a>
        <div class="flex items-center space-x-6 rtl:space-x-reverse">
          <span class="text-sm text-body">
            Welcome, <span class="font-medium text-heading">{{ userDisplay }}</span>
          </span>
          <button
            v-if="!authStore.isAuthenticated"
            @click="authStore.toggleAuthModal"
            class="text-sm font-medium text-fg-brand hover:underline transition-colors"
          >
            Login
          </button>
          <button
            v-else
            @click="handleLogout"
            class="text-sm font-medium text-fg-brand hover:underline transition-colors"
          >
            Logout
          </button>
        </div>
      </div>
    </nav>
  </header>
</template>
